...
export function MobileMenu({ children }: PropsWithChildren<{}>) {
  ...
  return (
    <Dialog as="div" className="lg:hidden" open={isOpen} onClose={setIsOpen}>
      ...
      <DialogPanel className="fixed border-t border-t-red-100 top-[5.75rem] inset-y-0 right-0 w-full overflow-y-auto bg-white py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10 z-20">
        {children}
      </DialogPanel>
    </Dialog>
  );
        ...
        >
          <DisclosureButton
            className={clsx(
              ...
              open && 'text-red-600',
              !open && 'text-gray-900',
              open && nestLevel === 1 && 'bg-red-100',
              !open && nestLevel === 1 && 'border-b border-b-red-100',
          ...
          >
            {title}
            <ChevronDownIcon
              className={clsx('h-5 w-5 flex-none text-red-600', {
                'rotate-180': open,
              })}
              aria-hidden="true"
            />
...
function HamburgerLogo() {
  return (
    ...
    >
      <path d="M5.03394 0.00125289H19.8146C21.3951 -0.0663573 21.3951 2.62517 19.8146 2.55756H5.09285C3.54107 2.62517 3.48072 0.00125289 5.0325 0.00125289H5.03394ZM19.8146 10.2233C21.3348 10.2233 21.3348 12.778 19.8146 12.778H11.481C9.92922 12.778 9.92922 10.2233 11.481 10.2233H19.8146ZM19.8146 20.4453C21.3348 20.4453 21.3348 23 19.8146 23H1.14013C-0.380039 23 -0.380039 20.4453 1.14013 20.4453H19.8146Z" />
    </svg>
  );
}
